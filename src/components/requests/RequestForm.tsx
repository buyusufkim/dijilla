...
export function RequestForm({ bootstrap, initialKind = 'roadside', onCreated }: { bootstrap: any; initialKind?: typeof requestKinds[number]; onCreated: (id: string) => void }) {
  ...
  return <section className="space-y-4 rounded-2xl border border-white/10 bg-[#1A233A] p-5">
    ...
    {!settings.requests_enabled || (settings.enabled_kinds && !settings.enabled_kinds.includes(kind)) ? <div role="status" className="rounded-xl border border-amber-300/15 bg-amber-300/5 p-5"><p className="font-medium text-amber-200">Bu hizmet için talep alımı şu anda kapalı.</p><p className="mt-2 text-sm text-slate-400">Açıldığında başvurunuzu bu ekrandan iletebilirsiniz.</p>{bootstrap.admin&&<a href="/settings" className="mt-4 inline-block text-sm text-cyan-300 underline">Uygulama ayarlarını aç</a>}</div> : <form onSubmit={submit} key={kind} className="space-y-5">
      {(settings.business_name||settings.support_hours||settings.support_email)&&<div className="rounded-xl border border-white/10 p-4 text-sm"><p className="font-medium">{settings.business_name}</p><p className="mt-1 text-slate-400">{settings.support_hours}</p>{settings.support_email&&<a href={`mailto:${settings.support_email}`} className="mt-2 inline-block text-cyan-300">{settings.support_email}</a>}</div>}
